---
title: Catálogo de Produtos
module: cadastros
route: /product
pdfRef: '2.4'
videoUrl: /videos/cadastros-catalogo-de-produtos-v2.mp4
updatedAt: '2026-02-27'
tags:
  - produtos
  - comercial
  - estoque
permissionHints:
  - product.list
order: 12
version: v2
description: 'Registre produtos para venda, estoque e faturamento.'
menuGroup: cadastros
menuGroupLabel: Cadastros
related:
  - comercial/pedidos-de-venda
  - gestao/hub-de-estoque
---
## Quando usar
Registre produtos para venda, estoque e faturamento.

## Pré-requisitos
- Perfil com permissões indicadas em `Permissões necessárias`.
- Dados mínimos já cadastrados para o fluxo.
- Acesso ao módulo em `/product`.

## Passo a passo
1. Acesse o menu correspondente ao módulo `Cadastros`.
2. Abra a funcionalidade `Catálogo de Produtos`.
3. Execute o fluxo principal seguindo a sequência operacional da tela.
4. Revise os dados antes de confirmar a operação.

## Campos críticos
- Campos obrigatórios exibidos com validação de preenchimento.
- Identificadores de cliente/equipamento/produto conforme contexto.
- Datas, status e responsáveis quando aplicável.

## Erros comuns
- Permissão insuficiente para a ação solicitada.
- Dados obrigatórios ausentes ou em formato inválido.
- Inconsistência de vínculo entre registros relacionados.

## Permissões necessárias
- product.list

## Relacionados
- [comercial/pedidos-de-venda](/docs/comercial/pedidos-de-venda)
- [gestao/hub-de-estoque](/docs/gestao/hub-de-estoque)


## O que você pode fazer nesta página
- Listar e filtrar registros.

## Fluxo detalhado
1. Acesse o menu Cadastros e abra Catálogo de Produtos.
2. Use a busca/filtros da listagem para localizar registros.
7. Revise mensagens de validação antes de concluir.

## Campos e validações
- Consulte o PDF seção 2.4 para nomes de campos, obrigatoriedade, máscaras e exemplos específicos.
- Respeite formatos: datas em yyyy-mm-dd; e-mails válidos; documentos com dígitos completos.
- Campos marcados com * são obrigatórios na criação e edição, quando exibidos.

## Permissões necessárias (nomes no sistema)
- Listar produtos

## Conteúdo detalhado (PDF 2.4 Catalogo de Produtos)


